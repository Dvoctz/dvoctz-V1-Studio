...
export const SportsDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const fetchData = useCallback(async <T extends EntityName>(entityName: T, force: boolean = false): Promise<SportsState[T]> => {
        if (!force && state[entityName] !== null) {
        ...
        try {
            ...
            if(entityName === 'fixtures') query = query.order('date_time');
    ...
    const prefetchAllData = useCallback(async () => {
        ...
        await Promise.all(allEntities.map(e => fetchData(e, true)));
    }, [fetchData]);
    ...
    useEffect(() => {
        ...
        const allTableNames: EntityName[] = ['tournaments', 'clubs', 'teams', 'players', 'fixtures', 'sponsors', 'tournamentSponsors', 'playerTransfers', 'notices', 'rules', 'tournamentRosters', 'tournamentTeams', 'tournamentAwards'];

        allTableNames.forEach(entityName => {
            const tableName = entityName === 'rules' ? 'game_rules' : entityName;
            const channel = supabase.channel(`public:${tableName}`)
                .on('postgres_changes', { event: '*', schema: 'public', table: tableName }, (payload) => {
                    console.log(`Realtime change on ${tableName}, refetching...`, payload);
                    fetchData(entityName, true); // Force refetch on any change
                })
                .subscribe((status, err) => {
                    if (status === 'SUBSCRIBED') console.log(`Subscribed to ${tableName}`);
                    else if (status === 'CHANNEL_ERROR') console.error(`Subscription error on ${tableName}:`, err);
                });
    ...
    // --- CRUD Operations --- //
    // Each operation will now force a refetch of the relevant data upon success.
    ...
    const updateRules = async (content: string) => {
        const { data, error } = await supabase.from('game_rules').upsert({ id: 1, content }).select().single();
        if (error) throw error;
        setState(s => ({...s, rules: mapRules(data)}));
    };
    ...
    const bulkAddOrUpdatePlayers = async (csvPlayers: CsvPlayer[]) => {
        ...
        for (const p of csvPlayers) {
            ...
            if (!clubId && !teamId) continue; // Skip if no affiliation
            ...
            playersToUpsert.push({ 
                ...
                club_id: clubId, 
                role: 'Main Netty', 
                stats: { matches: Number(p.matches) || 0, aces: Number(p.aces) || 0, kills: Number(p.kills) || 0, blocks: Number(p.blocks) || 0 } 
            });
        }
        if(playersToUpsert.length > 0) {
            await supabase.from('players').upsert(playersToUpsert, { onConflict: 'name' });
        }
        await fetchData('players', true);
    };
    ...
    const bulkUpdatePlayerTeam = async (playerIds: number[], teamId: number | null) => {
        let clubId = null;
        if (teamId && state.teams) {
            clubId = state.teams.find(t => t.id === teamId)?.clubId || null;
        }
        const { error } = await supabase.from('players').update({ team_id: teamId, club_id: clubId }).in('id', playerIds);
        if (error) throw error;
        await fetchData('players', true);
    };
    ...
    const concludeLeaguePhase = async (tournamentId: number) => {
        ...
        if (fixturesPayload.length > 0) {
            await bulkAddFixtures(fixturesPayload);
        }

        await updateTournament({ ...state.tournaments.find(t=>t.id===tournamentId)!, phase: 'knockout' });
    };
    ...
    const getActiveNotice = useCallback(() => {
        ...
        const now = new Date();
        return state.notices.filter(n => new Date(n.expiresAt) > now).sort((a, b) => {
    ...
    const getStandingsForTournament = useCallback((tournamentId: number): TeamStanding[] => {
        ...
        const tournamentTeams = (state.tournamentTeams || []).filter(tt => tt.tournamentId === tournamentId).map(tt => tt.teamId);
        
        const teamsInTournament = (state.teams || []).filter(t => tournamentTeams.includes(t.id));
        ...
        tournamentFixtures.forEach(f => {
            const team1InStandings = standingsMap.has(f.team1Id);
            const team2InStandings = standingsMap.has(f.team2Id);

            if (!f.score || !team1InStandings || !team2InStandings) return;
    ...
    }, [state.fixtures, state.teams, state.tournamentTeams, getFixturesByTournament]);
